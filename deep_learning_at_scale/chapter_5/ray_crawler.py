...
ray_crawler = typer.Typer(no_args_is_help=True, pretty_exceptions_enable=False)

"""
This example is for chapter 3 and is designed to show 
how to parallise a workload using thread and processes. 

LinkFinder and TextMatcher are two module that are designed to crawl arxiv website
for link to abstracts and extract sentences with deep learning and full names 
from the abstract pages of the arxiv. 

To run this sample, use following command:

# To get links from arxiv - this will create a links.json file
deep-learning-at-scale chapter_3 crawler get-links

# To extract sentences with deep learning and full names from the pages 
# use following. Use --max-workers to adjust the parallisation. 
...
def ray_parallelised_sentences(
    links,
    phase_pattern: str,
):
    sentence_matcher = TextMatcher.remote(phase_pattern=phase_pattern)
    futures = [sentence_matcher.find_matched_sentences.remote(link) for link in links]
    values = ray.get(futures)
    all_sentences, all_names = list(zip(*values))
...
def extract_sentences(
    ...
    sentences, names = ray_parallelised_sentences(links, phase_pattern)
